...
class TestJSONOperations:
    ...
    @pytest.mark.asyncio
    async def test_json_set_success(self, mock_redis_connection_manager, sample_json_data):
        ...
        mock_redis.json.return_value.set.assert_called_once_with("test_doc", "$", sample_json_data)
        assert "JSON value set at path '$' in 'test_doc'." in result

    @pytest.mark.asyncio
    async def test_json_set_with_expiration(self, mock_redis_connection_manager, sample_json_data):
        ...
        mock_redis.json.return_value.set.assert_called_once_with("test_doc", "$.name", "John Updated")
    ...
    async def test_json_set_nested_path(self, mock_redis_connection_manager):
        ...
        mock_redis.json.return_value.set.assert_called_once_with("test_doc", "$.user.profile.age", 25)
    ...
    async def test_json_set_redis_error(self, mock_redis_connection_manager):
        ...
        mock_redis = mock_redis_connection_manager
        mock_redis.json.return_value.set.side_effect = RedisError("JSON module not loaded")
        
        result = await json_set("test_doc", "$", {"key": "value"})
        
        assert "Error setting JSON value at path '$' in 'test_doc': JSON module not loaded" in result

    @pytest.mark.asyncio
    async def test_json_get_success(self, mock_redis_connection_manager, sample_json_data):
    ...
    @pytest.mark.asyncio
    async def test_json_get_default_path(self, mock_redis_connection_manager, sample_json_data):
    ...
    async def test_json_get_redis_error(self, mock_redis_connection_manager):
        ...
        assert "Error retrieving JSON value at path '$' in 'test_doc': Connection failed" in result
    ...
    async def test_json_del_success(self, mock_redis_connection_manager):
        ...
        mock_redis.json.return_value.delete.assert_called_once_with("test_doc", "$.name")
    ...
    async def test_json_del_redis_error(self, mock_redis_connection_manager):
        ...
        mock_redis = mock_redis_connection_manager
        mock_redis.json.return_value.delete.side_effect = RedisError("Connection failed")
        
        result = await json_del("test_doc", "$.name")
        
        assert "Error deleting JSON value at path '$.name' in 'test_doc': Connection failed" in result
    ...
    async def test_json_set_with_array(self, mock_redis_connection_manager):
        ...
        mock_redis.json.return_value.set.assert_called_once_with("test_doc", "$.items", array_data)
    ...
    async def test_json_set_with_nested_object(self, mock_redis_connection_manager):
        ...
        mock_redis.json.return_value.set.assert_called_once_with("test_doc", "$", nested_data)
    ...
    async def test_json_get_array_element(self, mock_redis_connection_manager):
        ...
        mock_redis.json.return_value.get.assert_called_once_with("test_doc", "$.items[0]")
        assert result == ["first_item"]

    @pytest.mark.asyncio
    async def test_json_operations_with_numeric_values(self, mock_redis_connection_manager):
    ...
    @pytest.mark.asyncio
    async def test_json_operations_with_boolean_values(self, mock_redis_connection_manager):
        ...
        mock_redis.json.return_value.set.assert_called_with("test_doc", "$.active", True)
    ...
    async def test_json_set_expiration_error(self, mock_redis_connection_manager):
        ...
        assert "Error setting JSON value at path '$' in 'test_doc': Expire failed" in result
    ...
    async def test_json_del_multiple_matches(self, mock_redis_connection_manager):
        ...
        mock_redis = mock_redis_connection_manager
        mock_redis.json.return_value.delete.return_value = 3  # Multiple elements deleted
    ...
    @pytest.mark.asyncio
    async def test_json_operations_with_null_values(self, mock_redis_connection_manager):
        ...
        mock_redis.json.return_value.set.assert_called_with("test_doc", "$.optional_field", None)

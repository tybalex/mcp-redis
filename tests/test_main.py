...
class TestCLI:
    ...
    def test_cli_with_url_parameter(self, mock_server_class, mock_set_config, mock_parse_uri):
    ...
    def test_cli_with_ssl_parameters(self, mock_server_class, mock_set_config):
        ...
        result = self.runner.invoke(cli, [
            ...
            '--ssl-cert-reqs', 'optional',
            '--ssl-ca-certs', '/path/to/ca-bundle.pem'
        ])
    ...
    @patch('src.main.set_redis_config_from_cli')
    @patch('src.main.RedisMCPServer')
    def test_cli_url_overrides_individual_params(self, mock_server_class, mock_set_config, mock_parse_uri):
